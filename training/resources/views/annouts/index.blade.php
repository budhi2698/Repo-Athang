	...
	<body>
		...
		<div class="container mt-2">
			<div class="row">
				<div class="col-lg-12 margin-tb">
					...
					<div class="pull-right mb-2">
						<a class="btn btn-success" href="{{ route('annouts.create') }}">Add</a>
					</div>
				</div>
			</div>
			...
			<table class="table">
				...
				<tbody>
						@foreach ($annouts as $annout)
						<tr>
							<td>{{ $annout->id }}</td>
							<td>{{ $annout->title }}</td>
							<td>
								<img src="{{ Storage::url($annout->image) }}" alt="{{ $annout->title }}" width="100">
							</td>
							<td>{{ $annout->description }}</td>
							<td>
								<a href="{{ route('annouts.edit', $annout->id) }}" class="btn btn-primary">Edit</a>
								<form action="{{ route('annouts.destroy', $annout->id) }}" method="POST" style="display: inline-block;">
									@csrf
									@method('DELETE')
